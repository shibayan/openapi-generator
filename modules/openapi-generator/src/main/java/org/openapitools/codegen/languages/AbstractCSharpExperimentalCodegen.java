...
public abstract class AbstractCSharpExperimentalCodegen extends DefaultCodegen implements CodegenConfig {
    ...
    protected String packageReleaseNotes;
    ...
    protected Set<String> valueTypes;
    protected Set<String> collectionTypes;
    protected Set<String> mapTypes;

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractCSharpExperimentalCodegen.class);

    public AbstractCSharpExperimentalCodegen() {
        super();
        ...
        collectionTypes = new HashSet<String>(
                Arrays.asList("IList", "ICollection", "IReadOnlyList", "IEnumerable")
        );

        mapTypes = new HashSet<String>(
                Arrays.asList("IDictionary", "IReadOnlyDictionary")
        );
    ...
    public void processOpts() {
        ...
        if (additionalProperties.containsKey(CodegenConstants.LICENSE_ID)) {
            ...
            additionalProperties.put(CodegenConstants.LICENSE_ID, this.licenseId);
        }
        ...
        // {{packageTags}}
        if (additionalProperties.containsKey(CodegenConstants.PACKAGE_TAGS)) {
            setPackageTags((String) additionalProperties.get(CodegenConstants.PACKAGE_TAGS));
        } else {
            additionalProperties.put(CodegenConstants.PACKAGE_TAGS, packageTags);
        }

        // {{packageReleaseNotes}}
        if (additionalProperties.containsKey(CodegenConstants.PACKAGE_RELEASE_NOTES)) {
            setPackageReleaseNotes((String) additionalProperties.get(CodegenConstants.PACKAGE_RELEASE_NOTES));
        } else {
            additionalProperties.put(CodegenConstants.PACKAGE_RELEASE_NOTES, packageReleaseNotes);
        }
    ...
    public String toInstantiationType(Schema schema) {
        if (ModelUtils.isMapSchema(schema)) {
            ...
            String inner = getTypeDeclaration(additionalProperties);
            return instantiationTypes.get("map") + "<string, " + inner + ">";
        } else if (ModelUtils.isArraySchema(schema)) {
            ArraySchema arraySchema = (ArraySchema) schema;
            String inner = getTypeDeclaration(getSchemaItems(arraySchema));
    ...
    public String getTypeDeclaration(Schema schema) {
        if (ModelUtils.isMapSchema(schema)) {
            Schema additionalProperties = getAdditionalProperties(schema);
            String inner = getTypeDeclaration(additionalProperties);
            return typeMapping.get("map") + "<string, " + inner + ">";
        } else if (ModelUtils.isArraySchema(schema)) {
            ArraySchema arraySchema = (ArraySchema) schema;
            String inner = getTypeDeclaration(getSchemaItems(arraySchema));
            return typeMapping.get("array") + "<" + inner + ">";
        } else {
    ...
    public void setPackageReleaseNotes(String packageReleaseNotes) {
        this.packageReleaseNotes = packageReleaseNotes;
    }
